...
function EditSemuaPerangkatDesa() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await customApi.get(`perangkats/${id}`);
        setFormData({
          nama: res.data.nama,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await customApi.put(`perangkats/${id}`, data, {
        headers: {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      ...
      <div className="w-full max-w-lg bg-white shadow-md rounded-lg p-6">
        <h1 className="text-2xl font-semibold text-gray-800 mb-4">Edit Perangkat Desa</h1>
...
export default EditSemuaPerangkatDesa;
