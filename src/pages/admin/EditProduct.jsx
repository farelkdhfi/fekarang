...
function EditProduct() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setFormData({
          namaProduct: res.data.namaProduct,
          deskripsiProduct: res.data.deskripsiProduct,
          hargaProduct: res.data.hargaProduct,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await customApi.put(`products/${id}`, data, {
        headers: {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      ...
      <div className="w-full max-w-lg bg-white shadow-md rounded-lg p-6">
        <h1 className="text-2xl font-semibold text-gray-800 mb-4">Edit Produk</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Kontak Pemilik Produk</label>
            <input
              type="text"
              name="kontak"
