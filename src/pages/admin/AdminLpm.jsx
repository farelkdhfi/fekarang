...
function AdminLpm() {
  ...
  const fetchBeritas = async (page) => {
    try {
      const res = await customApi.get(`/lpms?page=${page}&limit=6`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await customApi.delete(`lpms/${id}`, {
        headers: {
    ...
    <section className="text-sm lg:text-base">
      <div className="py-10 px-6 lg:px-20">
        <div className="flex gap-3 justify-center">
          <h1 className="text-xl font-bold text-blue-700 font-montserrat mb-10">Badan LPM Desa</h1>
        </div>
        <Link to="/uploadlpm" className="flex items-center pb-5 gap-3 text-xl font-bold">
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between mt-4">
                  <button
                    onClick={() => navigate(`/lpms/${berita._id}/edit`)}
