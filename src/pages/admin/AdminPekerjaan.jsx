...
import Pendu from '../../assets/admin/pekerjaan.png'
import customApi from '../../api';

const AdminPekerjaan = () => {
    ...
    useEffect(() => {
        const fetchPenduduks = async () => {
            try {
                const res = await customApi.get('/pekerjaans');
    ...
    const handleEdit = (pendudukId) => {
        navigate(`/pekerjaans/${pendudukId}/edit`);
    };
            ...
            {penduduks.map((penduduk) => {
                return (
                    <div key={penduduk._id} className='flex flex-col bg-white shadow-md w-fit p-3 rounded-xl justify-center items-center border border-gray-200'>
                        <h1 className='text-sm font-bold text-blue-700'>Pekerjaan Penduduk</h1>
